...
この後も活性化関数を詳しく説明します。パーセプトロンは人間の発明です。そしてパーセプトロンを期待通りの動作をしてもらうためには様々な工夫を施しています。その中、活性化関数もその一つです。活性化関数を用いることで、パーセプトロン（或は人工ニューロン）にある特定の（人間にとって都合の良い）振る舞いをしてもらえるようになります。どんな活性化関数するには、それは研究者の様々な研究の結果によって、それぞれの特性を見て選択するものです。
...
順伝播は出力層から層を辿って活性化関数など計算していき、損失関数までの結果を出す。誤差逆伝播は、連鎖律で、損失関数の結果を用いて、順伝播と逆の方法で、パラメータを更新していく。方向が違うだけではなく、担当する作業も違う。
