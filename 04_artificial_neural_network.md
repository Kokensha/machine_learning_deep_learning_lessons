...
* データから自動で学習可能 -> 線形分離可能な問題に限られている（パーセプトロンの収束定理） -> 多層パーセプトロン

* 非線形分離問題は（自動で）学習することができない
...
* 損失関数は（人間、ニューラルネットワークを設計した人が）決めるもの

* 人口ニューラルネットワークの学習はデータから、自動的に、損失関数が最小になる重みパラメータを探し出すこと

* 損失関数は「指標、基準」、損失関数の最小値を探すための手法は勾配法
